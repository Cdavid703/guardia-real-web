...
export default function AdminIntegrantesPage() {
  ...
  const handleApproveRequestedRole = async (u: UserProfile) => {
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-400 text-sm mt-1">
          Solicitudes de ingreso, registro de usuarios y asignación de roles
        </p>
      </div>
        ...
        <div className="mb-10">
          <div className="mb-4">
            <h2 className="font-display text-navy text-xl font-bold uppercase tracking-wider flex items-center gap-2">
              ...
              Roles solicitados pendientes de aprobación
            </h2>
            <p className="text-gray-400 text-xs mt-0.5">
              Usuarios que se registraron y solicitaron un rol distinto a visitante
            </p>
          </div>
            ...
            {pendingApprovals.map(u => (
              <div key={u.uid} className="card p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  {u.photoURL ? (
                    <Image src={u.photoURL} alt={u.displayName} width={36} height={36} className="rounded-full shrink-0" />
                  ) : (
                  ...
                  <div>
                    <p className="text-sm font-semibold text-dark">{u.displayName}</p>
                    <p className="text-xs text-gray-400">{u.email}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="badge bg-amber-100 text-amber-700">
                    Solicita: {getRoleLabel(u.requestedRole as UserRole)}
                  </span>
                  <button
                    onClick={() => handleApproveRequestedRole(u)}
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-navy text-white text-xs font-bold uppercase tracking-wider">
                  ...
                  <th className="px-4 py-3 text-left">Rol solicitado</th>
